...
export default function UserCard({ admin, profile }: Props) {
    ...
    return (
        <div className="border-blue-300 border rounded-lg p-3 hover:bg-blue-100">
            {changing ? <input type="email" name="email" id="email" value={tmpProf.email} onChange={w => setTmpProf(curr => ({ ...curr, email: w.target.value }))} /> : <p className="text-xl font-bold">{prof.id}. {prof.email}</p>}
            {changing ? <input type="text" name="name" id="name" value={tmpProf.email} onChange={w => setTmpProf(curr => ({ ...curr, email: w.target.value }))} /> : <p>{prof.name} {prof.surname}</p>}
